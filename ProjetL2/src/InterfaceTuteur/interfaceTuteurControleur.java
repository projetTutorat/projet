...
import InterfaceScolarite.interfaceAttributionSalle;
import Modele.*;
...
public class interfaceTuteurControleur implements Initializable  {
    ...
    @FXML private TreeView<String> treeViewTutoratsInscritsTuteur;
	...
	/**
     * La méthode inscriptionTutoratTuteur n'est pas static et elle ne retourne rien.
     * Elle permet de s'inscrire a un tutorat en tant que tuteur
     * @param event
     *          event
     */
    ...
    /**
     * La méthode appelEtudiant n'est pas static et elle ne retourne rien.
     * Elle permet a un tuteur de faire l'appel des etudiants presents
     * @param event
     *          event
     * @throws IOexception       
     */
    public  void appelEtudiant(MouseEvent event) throws IOException {
        ...
        confirmation.getDialogPane().setContentText("Faire l'appel");
        confirmation.getDialogPane().setHeaderText("Appel etudiants");

        Optional<ButtonType> result = confirmation.showAndWait();
        if (result.get()==ButtonType.OK){
            Stage interfaceAppel = new Stage();
            new interfaceAppel().start(interfaceAppel);
    ...
    /**
     * La méthode getSeance n'est pas static et retourne le parametre seance.
     * Elle permet d'obtenir les informations d'une seance
     */
    public Seance getSeance() {
        TreeItem<String> item =  treeViewTutoratsInscritsTuteur.getSelectionModel().getSelectedItem();
        String date= String.valueOf(item).substring(29,39);
        String horaire=String.valueOf(item).substring(43,51);
        String besoin=(String.valueOf(item).split("   Besoin: ")[1]);
        String besoin2=(besoin.split("   Salle: ")[0]).substring(0,besoin.split("   Salle: ")[0].length()-1);
    ...
    public static interfaceTuteurControleur Instance;

    public interfaceTuteurControleur(){
        Instance = this;
    }
